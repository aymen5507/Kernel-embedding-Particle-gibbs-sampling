...
%% Learning parameters
K = 100; % number of models to be sampled
K_b = 200; % length of burn-in period
k_d = 5; % number of models to be skipped to decrease correlation (thinning)
...
%% Basis functions and priors - based on reduced-rank GP approximation
n_basis_x = [5 5]; % number of basis functions for each state x(i)
n_basis_u = 5; % number of basis functions for u
n_basis = [n_basis_u n_basis_x];
L_u = 10; % interval length for u
l_x = 20; % interval length for x
L_x = [l_x l_x]; 
L = zeros(1,1,n_z);
L(:) = [L_u L_x];

% Hyperparameters of squared exponential kernel
l = 2; % length scale
sf = 100; % scale factor
...
%% Set up basis functions and priors
n_basis_tot = prod(n_basis); % total number of basis functions
A_init = zeros(n_x,n_basis_tot);

% Matrices needed to construct the basis functions phi
jv = zeros(n_basis_tot,1,n_z); 
lambda = zeros(n_basis_tot,n_z); 

% Needed to construct the Cartesian product
Omega = cell(1,n_z);
for i=1:n_z
    Omega{i} = 1:n_basis(i);
end

% Number of variants with lower index - needed to construct Cartesian
% product
variants = [1 cumprod(n_basis(1:end-1))];

% Construct Cartesian product and calculate spectral densities
for i = 1:n_basis_tot
    % Calculate index of all sets in Omega
    remaining = i-1;
    for j = n_z:-1:1
        indices(j) = floor(remaining/variants(j))+1;
        remaining = rem(remaining,variants(j));
    end

    for j = 1:n_z
        jv(i,1,j) = Omega{j}(indices(j));
    end
    
    % Eigenvalues of Laplace operator
    lambda(i,:) = (pi.*squeeze(jv(i,1,:))./(2*squeeze(L))).^2;
end

jv = flip(jv,3);

% Basis functions
phi = @(x,u) prod(repelem(L.^(-1/2),n_basis_tot,size(u,2),1).*sin(pi*repelem(jv,1,size(u,2),1).*repelem((permute([u; x],[3 2 1])+L)./repelem(2*L,1,size(u,2),1),n_basis_tot,1,1)),3);

% Calculate left covariance matrix
V = build_covariance_matrix(lambda,l,sf);
...
%% Overload phi function
% This step is necessary as the standard phi function (defined inline above) 
% cannot be used with CasADi MX objects. The function is only used for the 
% optimization and not the learning since it is less efficient.
phi = @(x,u) phi_MX(n_basis_tot,n_z,L,jv,x,u);
